...
def create_app():
    ...
    @app.route('/reset')#creates a database and resets it
    def reset():
        DB.drop_all()
        DB.create_all()
    ...
    def compare(message=''):
        user1, user2 = sorted([request.values['user1'],#flask request
                               request.values['user2']])
        if user1 == user2:#in case user is compared to itself
